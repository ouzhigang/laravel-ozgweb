...
$(function() {
	...
	function show_data() {
		...
		do_show_data = false;
		var url = $("#cfg_url").val() + "/get_news_list";
		$.getJSON(
			url,
			{
				"page": page
			...
			function(data) {
				if(data.code == 0) {
					...
					for(var i in data.data.list) {
						html += '<li>';
						html += '<img src="' + $("#cfg_url").val() + '/images/dot2.gif" />';
						html += '<a href="' + $("#cfg_url").val() + '/news_view?id=' + data.data.list[i].id + '" target="_blank">' + data.data.list[i].name + '</a>';
