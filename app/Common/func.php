...
function page_count($count, $page_size) {
	if ($count % $page_size == 0) {
		...
		return intval(floor($count / $page_size) + 1);
	}
}
...
//两位小数的范围随机
function rand_float_2($min = 0, $max = 10) {
...
function is_wap_client() {
	$clientkeywords = array('nokia', 'sony', 'ericsson', 'mot', 'samsung',
		...
		'operamobi', 'openwave', 'nexusone', 'cldc', 'midp', 'wap', 'mobile', 'MicroMessenger'); 
    // 从HTTP_USER_AGENT中查找手机浏览器的关键字
    if (preg_match("/(" . implode('|', $clientkeywords) . ")/i", strtolower(request()->header("User-Agent")))) {
		return true;
	}
	return false;
}
...
function is_alipay_browser() {
	//判断是否使用支付宝浏览器
	$user_agent = request()->header("User-Agent");
	return !(strpos($user_agent, "AlipayClient") === false);
}
...
function is_robot() {
	if(!defined('IS_ROBOT')) {
		...
		if(!str_exists(request()->header("User-Agent"), 'http://') && preg_match("/($kw_browsers)/i", request()->header("User-Agent"))) {
			define('IS_ROBOT', false);
		}
		elseif(preg_match("/($kw_spiders)/i", request()->header("User-Agent")))
...
function http_post($url, $postdata) {
	...
	curl_setopt($ch, CURLOPT_POSTFIELDS, !is_array($postdata) ? $postdata : http_build_query($postdata));
